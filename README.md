...
**speech recognition**, sends it to **Claude**, and **speaks the reply back** — all over the
phone's loudspeaker.

> **Architecture chosen:** cellular + speakerphone, hybrid speech stack (on-device STT/TTS,
> Claude for the replies). See the honest limitations below before you rely on this.
...
│   ├── ConversationManager.kt   # greeting -> listen -> Claude -> speak -> repeat
...
│   ├── ClaudeClient.kt          # Anthropic Messages API client (OkHttp)
...
speech recognition and text-to-speech, and the default persona instructs Claude to reply in Nepali
(falling back to the caller's language if they clearly use another). Claude handles Nepali text
natively.
...
**3. Get a Nepali ggml model.** whisper.cpp needs a `ggml-*.bin` model. Options:
   - Convert the fine-tuned Nepali Whisper-small model
     (`amitpant7/Nepali-Automatic-Speech-Recognition` on Hugging Face) to ggml with whisper.cpp's
     `models/convert-h5-to-ggml.py`, then optionally quantize (`quantize model.bin model-q5.bin q5_0`)
     to shrink it and speed up inference.
   - Or start with the stock multilingual `ggml-small.bin` (or `ggml-base.bin` for speed) from
     whisper.cpp to sanity-check the pipeline; the fine-tuned model will transcribe Nepali better.

   Push it to the device and point the app at it:
```bash
adb push ggml-whisper-nepali.bin /sdcard/Download/ggml-whisper-nepali.bin
```
   Then in the app set **Whisper model path** to `/storage/emulated/0/Download/ggml-whisper-nepali.bin`
   (reading from shared storage may require the "Files and media" permission; the app's own files
   dir avoids that).
...
3. **In the app:**
   ...
   - Paste your **Claude API key** (`sk-ant-...`). Get one at <https://console.anthropic.com>.
...
|---|---|---|
| Model | `claude-haiku-4-5-20251001` (low latency) | `Prefs.DEFAULT_MODEL` |
...
The Claude API key is stored on-device in SharedPreferences and called directly from the phone.
...
STT → Claude → TTS with full-duplex control. The Android app then becomes a thin softphone/config
